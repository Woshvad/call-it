...
**Status:** IN PROGRESS — SAFETY-22 + 23 met live; CI safety-matrix + rituals captured (2026-06-04). **Settle BLOCKED — root cause found: the deployed Phase-2 ProfileRegistry lacks `globalRep()` that the Phase-6 SM calls (version mismatch); fix = cluster redeploy from current source. Release blocker.** Challenge (top-up), caller-exit (24h), dispute, drill, UAT PENDING.
...
| SAFETY-24 | ≥3 settled calls per type; outcome words, payouts, and rep updates verified | ⬜ BLOCKED (root cause found) | **ROOT CAUSE:** the deployed Phase-2 ProfileRegistry (`0xAfe239a3`, preserved/never-redeployed) lacks the `globalRep(address)` getter that the Phase-6 SettlementManager calls in `_computeRepDelta` (SettlementManager.sol:282) — and that call is OUTSIDE the Stylus try/catch, so settle reverts with NO data (selector absent). `applyRepDelta` DOES exist on the deployed PR (so createCall/seeding worked) — a partial-version mismatch. Pinpointed via forge fork trace (`test/SettleTrace.t.sol`, `-vvvv` → `ProfileRegistry::globalRep → [Revert]`). Event calls 3–7 correctly DEFER (5 SettlementDelayed, settle-1780600088911.jsonl). **FIX:** redeploy the cluster (ProfileRegistry + FFM/CE/SM — immutable PR refs cascade) from current source. Mainnet safe IFF its PR is current-source. **Release blocker — settlement is the core product.** |
