...
| SAFETY-26 | ≥1 full challenge cycle: propose → accept → settle duel | ⬜ PENDING | Seeder bug FIXED (c316667): Phase E now reads the real challengeId from the ChallengeProposed event (was a wrong `callId+100` guess → `acceptChallenge` reverted NotAuthorized 0xea8e4eb5). propose succeeded on the new cluster; full propose→accept→settle provable in the next funded run (wallets drained by the SAFETY-22/23/24 run). |
| SAFETY-27 | ≥1 dispute raised and owner-resolved | ⬜ PENDING | Now UNBLOCKED — 5 calls are settled on the new cluster (raiseDispute reverts `not-settled` pre-settlement; that precondition is now satisfiable). Provable in a targeted run. |
