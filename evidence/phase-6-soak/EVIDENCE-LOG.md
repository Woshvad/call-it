...
|------|-------------|--------|----------|
| SAFETY-21 | Soak duration ≥48 continuous hours with live relayer active | ✅ | **48h COMPLETE & clean — gate met 2026-06-10T08:45Z.** Authoritative window = Fly relayer continuous uptime: machine `83629da7359938` last `started` **2026-06-08T08:45:41Z**, still `started` at 2026-06-10T08:45:41Z → **48.0h elapsed, NO restart, clock never reset.** Official verdict — `bash apps/relayer/src/scripts/soak-status.sh` run **2026-06-10T08:45:52Z**: **"48h COMPLETE & healthy — ready for closeout (flip SAFETY-21)"** (window start 2026-06-08T08:45:41Z, 48h target 2026-06-10T08:45Z, elapsed 48.0h / remaining 0.0h, relayer health=ok, owner ok=**YES**, engine ok=**YES**). Supplementary read-only heartbeat `soak-21-heartbeat-1780996491.jsonl`: **72+ ticks @15-min, ZERO `"alert":true`, ZERO unhealthy** (every tick health=ok, owner_ok=true + engine_ok=true), TVL steady at 70000000 ($70). NOTE: the heartbeat logger only started 2026-06-09T09:14:51Z (a day into the soak) — it is SUPPLEMENTARY evidence, NOT the gate clock; the gate clock is the relayer continuous uptime above. Cluster: SM `0x2E26eEb3…97e7` owner=treasury `0xDa8c5726`, engine `0xe7e15980…`. |
...
| SAFETY-35 | Post-expiry follow gate: follow/fade revert with CallPastExpiry after expiry | FollowFadeMarket.t.sol | ✅ | `testPostExpiryGate()` (FollowFadeMarket.t.sol:143) — PASS by name 2026-06-07 (was mis-filed under SettlementSafetyMatrix). |
...
| SAFETY-40 | Rep decay / cooldown math: computeRepDelta returns expected values for given inputs | SolidityScoreEngine.t.sol + SettlementManagerTest + SettlementSafetyMatrix | ✅ | PASS by name 2026-06-07: rep-delta math `SolidityScoreEngine.t.sol` 6/6 parity (conviction 1/50/90/100 winner+loser, consensus-ignored); cold-start `testColdStartScale()` (SettlementManagerTest:182); caller-exit decay `test_callerExit_penalty_at50pct` + `_at15pct_floor` (SettlementSafetyMatrix). |
| SAFETY-41 | forceSettle cooldown: owner cannot call forceSettle before expiry+7d | SettlementManagerTest.sol | ✅ | `testForceSettleCooldown()` (SettlementManagerTest.sol:248) + `testForceSettleEvents()` (:262) — PASS by name 2026-06-07. |
...
- ✅ **Solidity version pinned** to `=0.8.30` (not `^0.8.24` floating)? (STACK.md) — verified 2026-06-07: `foundry.toml solc_version = "0.8.30"` + all 18 `src/**/*.sol` use exact `pragma solidity =0.8.30;` (0 floating pragmas).
