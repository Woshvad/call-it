...
| SAFETY-26 | ≥1 full challenge cycle: propose → accept → settle duel | ⬜ RE-RUN PENDING (proven on superseded cluster) | Proven 2026-06-05 on the **now-superseded** cluster (challenge #2 on call #12: propose tx `0xc02794a9…`, accept tx `0xc11b182c…`). The 2026-06-06 recovery cluster is empty of challenges → re-run via seeder Phase E or runbook §A (use a funded challenger ≠ caller; Phase E hardcodes wallet[2] which Phase A/B drains below the $5 MIN_STAKE). Not blocked on keys. |
