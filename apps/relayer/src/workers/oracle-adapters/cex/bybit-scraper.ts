/**
 * Bybit CEX listing scraper — per-exchange modular isolation (D-02, Pitfall 10).
 *
 * Scrapes https://announcements.bybit.com/en-US/ for token listings.
...
/** Bybit announcement page URL */
export const ANNOUNCE_URL = 'https://announcements.bybit.com/en-US/';

/**
 * Bybit exclusion patterns (Pitfall 19).
 */
export const EXCLUSION_PATTERNS: string[] = ['Innovation Zone'];

// ── Types ─────────────────────────────────────────────────────────────────────

export interface BybitScraperResult {
  detected: boolean;
  matchedTitle?: string;
...
function extractTitlesFromHtml(html: string): string[] {
  const titles: string[] = [];
  const patterns = [
    /<[^>]+class="[^"]*announcement-title[^"]*"[^>]*>([\s\S]*?)<\/[^>]+>/gi,
    /<h3[^>]*>([\s\S]*?)<\/h3>/gi,
    /<h2[^>]*>([\s\S]*?)<\/h2>/gi,
  ];
...
export class BybitScraper {
  constructor() {}

  parseHtmlForListing(html: string, tokenSymbol: string, tokenName: string): BybitScraperResult {
  ...
  private async scrapeWithPlaywright(): Promise<string> {
    ...
    try {
      ...
      logger.info({ event: 'bybit_scraper_page_loaded' }, 'Bybit page loaded');
      return html as string;
    } catch (err) {
      logger.error({ event: 'bybit_scraper_playwright_error', error: String(err) }, 'Bybit Playwright failed');
      return '';
    } finally {
  ...
  async detectListing(params: { tokenSymbol: string; tokenName: string; expiryTimestamp: number }): Promise<BybitScraperResult> {
    const logger = getLogger();
    logger.info({ event: 'bybit_scraper_attempt', ...params }, 'Bybit scraper started');
    const html = await this.scrapeWithPlaywright();
    const result = this.parseHtmlForListing(html, params.tokenSymbol, params.tokenName);
    logger.info({ event: 'bybit_scraper_result', detected: result.detected }, `Bybit scraper: ${result.detected ? 'FOUND' : 'NOT FOUND'}`);
    return result;
  }
}
