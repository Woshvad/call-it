/**
 * Bitget CEX listing scraper — per-exchange modular isolation (D-02, Pitfall 10).
 *
 * Scrapes https://www.bitget.com/blog for token listings.
 * - Multi-signal confirm: matches BOTH full tokenName AND tokenSymbol in post title (Pitfall 19)
 * - Exclusion patterns: 'PoP Zone', 'Margin Trading Only', 'Monitoring' (Pitfall 19)
...
/** Bitget announcement page URL */
export const ANNOUNCE_URL = 'https://www.bitget.com/blog';

/**
 * Bitget exclusion patterns (Pitfall 19).
 * 'PoP Zone' is Bitget's equivalent of Binance's Innovation Zone.
 */
export const EXCLUSION_PATTERNS: string[] = ['PoP Zone', 'Margin Trading Only', 'Monitoring'];

// ── Types ─────────────────────────────────────────────────────────────────────

export interface BitgetScraperResult {
  detected: boolean;
  matchedTitle?: string;
...
export class BitgetScraper {
  constructor() {}

  parseHtmlForListing(html: string, tokenSymbol: string, tokenName: string): BitgetScraperResult {
  ...
  private async scrapeWithPlaywright(): Promise<string> {
    ...
    try {
      ...
      logger.info({ event: 'bitget_scraper_page_loaded' }, 'Bitget page loaded');
      return html as string;
    } catch (err) {
      logger.error({ event: 'bitget_scraper_playwright_error', error: String(err) }, 'Bitget Playwright failed');
      return '';
    } finally {
  ...
  async detectListing(params: { tokenSymbol: string; tokenName: string; expiryTimestamp: number }): Promise<BitgetScraperResult> {
    const logger = getLogger();
    logger.info({ event: 'bitget_scraper_attempt', ...params }, 'Bitget scraper started');
    const html = await this.scrapeWithPlaywright();
    const result = this.parseHtmlForListing(html, params.tokenSymbol, params.tokenName);
    logger.info({ event: 'bitget_scraper_result', detected: result.detected }, `Bitget scraper: ${result.detected ? 'FOUND' : 'NOT FOUND'}`);
    return result;
  }
}
