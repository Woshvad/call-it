/**
 * OKX CEX listing scraper — per-exchange modular isolation (D-02, Pitfall 10).
 *
 * Scrapes https://www.okx.com/help/section/new-listings for token listings.
...
/** OKX announcement page URL */
export const ANNOUNCE_URL = 'https://www.okx.com/help/section/new-listings';

/**
 * OKX exclusion patterns (Pitfall 19).
 */
export const EXCLUSION_PATTERNS: string[] = ['Innovation Zone'];

// ── Types ─────────────────────────────────────────────────────────────────────

export interface OkxScraperResult {
  detected: boolean;
  matchedTitle?: string;
...
export class OkxScraper {
  constructor() {}

  parseHtmlForListing(html: string, tokenSymbol: string, tokenName: string): OkxScraperResult {
  ...
  private async scrapeWithPlaywright(): Promise<string> {
    ...
    try {
      ...
      logger.info({ event: 'okx_scraper_page_loaded' }, 'OKX page loaded');
      return html as string;
    } catch (err) {
      logger.error({ event: 'okx_scraper_playwright_error', error: String(err) }, 'OKX Playwright failed');
      return '';
    } finally {
  ...
  async detectListing(params: { tokenSymbol: string; tokenName: string; expiryTimestamp: number }): Promise<OkxScraperResult> {
    const logger = getLogger();
    logger.info({ event: 'okx_scraper_attempt', ...params }, 'OKX scraper started');
    const html = await this.scrapeWithPlaywright();
    const result = this.parseHtmlForListing(html, params.tokenSymbol, params.tokenName);
    logger.info({ event: 'okx_scraper_result', detected: result.detected }, `OKX scraper: ${result.detected ? 'FOUND' : 'NOT FOUND'}`);
    return result;
  }
}
