...
**Plans**: 5 plans across 4 waves (CORE first, FEED gated)
  ...
  - [x] 01.5-01-PLAN.md — Wave 1: [BLOCKING] setRelayer(relayerKms) on canonical PR 0xF66C0AFE (owner gate) + social-linking env surface — ✅ COMPLETE (2026-06-07): Task 1 (env surface) committed `3fbf88d`; Task 2 setRelayer EXECUTED by operator — derived oauth-proof KMS `0xdFc80922…` (scaffold `9b41f0f`), funded 0.05 ETH, broadcast `setRelayer` from treasury `0xDa8c5726`; on-chain `relayer()` == `0xdFc80922…` verified (was `0x0`). SUMMARY written.
  - [x] 01.5-02-PLAN.md — Wave 2: relayer link-proof service (Twitter/Privy + Farcaster/auth-client) + D-06 uniqueness + D-05 proofHash + KMS submit + unlink-purge + SocialUnlinked watcher + /api/profile enrichment
  ...
  - [x] 01.5-04-PLAN.md — Wave 3: link/unlink UI (onboarding + Settings) + Farcaster AuthKitProvider below PrivyProvider + AUTH-16 opt-in consent (D-14)
  - [x] 01.5-05-PLAN.md — Wave 4 [GATED]: checkpoint:human-verify (X API tier/follows.read + Neynar key) then "From your X / Farcaster" feed sections + viewer-only follow-graph data layer — ✅ COMPLETE (2026-06-06, complete-with-documented-deferral; SUMMARY written): Tasks 2-4 built + committed (`7960dce` data layer + clients, `d02e694` feed routes, `e1076a3` UI sections); 18 relayer tests green, relayer+web builds exit 0. Task 1 (X API tier/follows.read + Neynar key) RESOLVED via the plan's own degraded-to-empty disposition — v1 ships with NO keys; sections render the quiet empty state and activate with NO code change once `X_API_BEARER_TOKEN` + `NEYNAR_API_KEY` are set (AUTH-14 live data deferred; AUTH-15/16/17/18 satisfied + tested).
...
**Plans**: 7 plans across 6 waves
  ...
  - [x] 03-03-PLAN.md — Wave 3: DeployPhase3.s.sol + [OPERATOR] Sepolia deploy checkpoint + addresses.ts CHALLENGE_ESCROW_ARBITRUM_SEPOLIA
...
**Plans**: 9 plans across 6 waves
  ...
  - [x] 04-01-PLAN.md — Wave 1: TDD test scaffolds (SmTestHelper + SettlementManagerTest + FfmSettlementTest + SettlementDisputeTest + pyth/defillama/cex Vitest scaffolds + D-08 outcome-word RED gate)
  - [x] 04-02-PLAN.md — Wave 1: ISettlementManager interface + FFM redeploy (applySettlement + real claimPayout) + SettlementManager.sol (14-step settle + Solidity baseline rep + dispute + forceSettle) + ProfileRegistry stub fill
  ...
  - [x] 04-08-PLAN.md — Wave 5: Dispute modal (IPFS upload + bond) + Provenance proof modal (D-10 oracle URL/tx/raw data/EIP-712 sig) + /disputes/ public log + owner resolve admin with reversal preview
  - [x] 04-09-PLAN.md — Wave 6: CEX weekly CI cron + SAFETY-57 doc + addresses.ts finalization + end-to-end verification checkpoint
...
### Phase 05.1: Non-Pyth Oracle Rail Activation (INSERTED)

**Goal**: Make all 6 non-Pyth settlement paths (Snapshot, Tally, NFT-TWAP, DefiLlama, RPC, CEX) settle end-to-end on Arbitrum Sepolia — completing the functional rail that Phase 4 acknowledged-deferred (04-VERIFICATION.md "Acknowledged Gaps B"). The on-chain attestation rail (`submitAttestation` + EIP-712 + `ECDSA.recover` + per-type signer registry) is already implemented + security-verified, and the unified relayer signing core (`apps/relayer/src/workers/oracle-adapters/oracle-attestation.ts`) is built + tested; what remains is functional wiring + a governance schema change. **Inserted before Phase 6** so the schema-change redeploy is exercised by the ≥48h soak and is the final address set the multisig takes ownership of. Every failure path reverts or DEFERs (Pending) — never a forged/mis-settled outcome. The Pyth settlement spine is unaffected.
**Mode:** mvp
**Depends on**: Phase 5
**Requirements**: SETTLE-* non-Pyth oracle subset (Snapshot/Tally/NFT-TWAP/DefiLlama/RPC/CEX dispatch + EIP-712 attestation wiring — exact IDs finalized in discuss/plan) + new **SETTLE-DUAL-GOV** (support BOTH Snapshot and Tally for governance markets via distinct adapterMap keys)
**Success Criteria** (what must be TRUE):

  1. **Schema change**: SettlementManager `adapterMap` supports DISTINCT keys for Snapshot(4) and Tally(5) governance markets (e.g. `EventSubtype.Governance` split into `Governance_Snapshot` / `Governance_Tally`, or equivalent design from RESEARCH); the oracleType↔adapterMap binding (blast-radius limit, `SettlementManager.sol:591`) is preserved across the change.
  2. **Deploy config**: `setAdapterMap` (`SettlementManager.sol:622`) is called in the deploy script for EVERY configured `(marketType, eventSubtype)` pair; `cast call SM "adapterMap(uint8,uint8)"` returns non-zero adapters for all 6 non-Pyth paths; a Snapshot-routed and a Tally-routed governance call both settle (or DEFER on ambiguity) on Sepolia — neither reverts due to adapterMap misconfig.
  3. **Relayer rewire**: all 6 adapters + `settlement-watcher.ts` emit the unified EIP-712 attestation via `oracle-attestation.ts` — real `block.chainid` (421614 on Sepolia), real 19-field `Call` ABI decode, on-chain `adapterMap` dispatch; legacy `CallIt-*` per-adapter domains + hardcoded `chainId=42161` + the fabricated 14-field `Call` decode are removed.
  4. **Off-chain criteria**: adapter call-criteria retrieval resolves the string identifiers the on-chain `Call` does not carry (`protocolSlug`, `proposalId`, `tokenSymbol`); per-market `targetValue` units are verified against call-creation; any mismatch DEFERs rather than settles.
  5. **Redeploy rollup**: redeployed contract addresses written to `packages/shared/src/constants/addresses.ts`; CallRegistry / FollowFadeMarket / SettlementManager rewired to the new SM; subgraph updated + redeployed to Studio; Phase 4 money-path + settlement test suites re-pass against the new deploy.
...
- [x] 05.1-02-PLAN.md — Wave 2: DeployPhase5_1.s.sol — redeploy CR+SM, setAdapterMap x8, setAttestationSigner x6, Sepolia deploy checkpoint
...
- [x] 05.1-04-PLAN.md — Wave 2: Criteria store — callOracleCriteria Drizzle schema + migration + resolveCriteria/insertCriteria + adapter wiring
- [x] 05.1-05-PLAN.md — Wave 3: Redeploy rollup — addresses.ts, subgraph.yaml, subgraph redeploy, relayer env update, Phase 4 test re-pass, live Sepolia smoke
...
**Goal**: All safety invariants verified on Sepolia under load for ≥48 hours with seeded activity. Stylus destruction drill proves the Solidity baseline fallback actually runs under the runtime try/catch. TVL cap aggregation boundary tests cover the full CallRegistry + FollowFadeMarket + ChallengeEscrow surface. The full PITFALLS.md "Looks Done But Isn't" 38-item checklist must pass. **The Safe 2-of-3 multisig mechanism is REHEARSED on the Sepolia cluster: a Safe is deployed on Sepolia and ownership of the 6 Sepolia surfaces (CallRegistry, FollowFadeMarket, ChallengeEscrow, SettlementManager, ProfileRegistry, StylusScoreEngine proxy admin) is transferred to it via Ownable2Step, then Safe-gated pause + proxy upgrade are proven end-to-end. The PRODUCTION Arbitrum One Safe deploy + mainnet ownership transfer (Risk #2 close) is MOVED to the final Phase 10 per operator decision (2026-06-04).** Env diff ritual, chain ID grep in bundle, relayer ETH balance check, Pyth feed bytes32 pre-deploy CI verification all run here.
**Mode:** mvp
**Depends on**: Phase 5
...
**Success Criteria** (what must be TRUE):
  ...
  4. Safe 2-of-3 multisig is deployed on the **Sepolia rehearsal cluster**; ownership of the 6 Sepolia surfaces (CallRegistry, FollowFadeMarket, ChallengeEscrow, SettlementManager, ProfileRegistry, StylusScoreEngine `TransparentUpgradeableProxy` admin) is transferred via Ownable2Step `acceptOwnership`; `cast call <each> "owner()"` returns the Safe; Safe-gated pause + proxy upgrade proven end-to-end. The production Arbitrum One Safe deploy + mainnet ownership transfer (Risk #2 close) is **deferred to the final Phase 10**.
...
**Folded in (Phase 5 close — Bucket A live UAT)**: the ≥48h soak also closes the 5 deferred Phase-4 live-UAT items (04-HUMAN-UAT.md): (1) live settlement E2E, (2) dispute flow E2E, (3) provenance modal content (D-10), (4) OG card 200px readability QA (SHARE-12/UI-18), (5) live OG render for settled/exited calls. These need the live `call-it-relayer-sepolia` + a real settle cycle, which this phase provides.
**Plans**: TBD
**Pitfalls mitigated**: 2 (destruction drill closed), 3 (TVL cap aggregation boundary verified across all 3 contracts), 4 (Pyth feed pre-deploy CI), 5 (env config drift caught), 6 (multisig mechanism rehearsed on Sepolia; mainnet promotion deferred to Phase 10), 7 (KMS-sign integration verified end-to-end + secondary on-chain TWAP sanity-check), 9 (multi-call interference fuzz fixtures), 10 (Sepolia-load post-expiry settle latency monitored), 11 (Phase 6 dashboard surfaces self-fade rep farming patterns for v1.1 gate), 14 (operator runbook for paymaster cap-raising during launch verified), 20 (direct-tx bypass test confirms server-side 24h cooldown enforcement)
...
**Success Criteria** (what must be TRUE):
  ...
  5. Receipt page server-renders OpenGraph meta tags referencing the OG card URL with the `?v={statusVersion}` cache-buster; receipts are off-chain OG images referenced onchain by hash for verification (NOT NFTs); receipt page loads without auth (public-by-default per §18.1) with sign-in CTA prominent for unauthenticated viewers.**Plans**: 6 plans across 4 waves
...
**Wave 1**
  ...
  - [x] 07-02-PLAN.md — Wave 1: [BLOCKING] relayer statement column + migration + live-state marketLine (D-05) + subgraph Call.statement templated mirror (D-03, v0.9.0 build)
...
- [x] 08-03-PLAN.md — Wave 1 (Slice B): legacy Frame tx wire endpoint (status-aware buttons D-02/D-06/D-06a, origin-locked calldata,  one-tap D-07) + promoted challengeEscrowAbi — eip155:421614 wire well-formed (live broadcast = Phase 10, D-01)
...
- [x] 08-06-PLAN.md — GAP 2 (blank Mini App): add @farcaster/miniapp-sdk + call sdk.actions.ready() once mounted (splash dismiss) + render the read-only receipt without blocking on Privy/wagmi wallet init; tap-to-transact stays Phase 10 (D-01)
...
### Phase 09.2: Prototype design adoption — rebuild apps/web UI to the call-it-frontend design canon (tokens, brutal primitives, shell, all routes; runs BEFORE 09.1 demo hardening) (INSERTED)

**Goal:** Every user-facing surface of apps/web renders the canonical `call it frontend/` prototype design (root variant: Archivo/Inter/JetBrains Mono via next/font, cream #F5F1E8 inverse blocks, hard black offset shadows, radius 0, prototype `:root` tokens as the single token layer, brutal-* primitive classes). The prototype is a markup+token donor only — every screen keeps its existing hooks, data sources, guards (AUTH-44, D-09, provider-order AST lock, MiniAppReady, flexbox-only in OG/shared), and tx wiring. Adoption order: tokens/primitives/@call-it/ui retheme → app shell → leaderboard/profile/feed → /call/[id] live+settled receipt + modals → /new → duel + landing decision → uncovered surfaces (signin, onboarding, disputes, settings, notifications) → visual-golden regeneration + mobile clamp pass. Design data the backend can't supply yet (sparklines, calibration/ROI, pool $ on feed cards, ticker prices, platform totals) degrades to hidden — never faked.
...
- [x] 09.2-07-PLAN.md — Wave 4: /call/[id] settled receipt branch (stamp + SETTLED_OUTCOMES tints + 2-node timeline + share wiring kept) + Dispute/Provenance cream chrome
- [x] 09.2-08-PLAN.md — Wave 4: cream .modal-panel retheme — FollowFadeModal/CallerExitModal/PositionExitModal/ChallengeFormModal (logic untouched, D-06/D-13)
...
**Plans**: TBD
**Pitfalls mitigated**: 6 (multisig promoted on mainnet — single-key path finally dead)
...
| 1.5. Social linking | 3/5 | In Progress|  |
