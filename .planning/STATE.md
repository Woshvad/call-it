...
| 260611-fast | Composer live-price value highlighted in var(--accent-win) (label stays muted) — PriceTargetFields + SpreadVsFields LivePriceRow; web 236/236, build 0 | 2026-06-11 | 9bc2335 | — |
| 260611-if0 | Close out the two hog findings honestly (verified on-chain truth first: SettlementManager.sol:713-722 settles price-target strictly `currentPrice >= target` → CallerWon; contracts immutable → below-target calls IMPOSSIBLE in v1, must not be faked) — (1) direction honesty: mono helper "Wins if price closes at or above target at the deadline." under Price Target, chip-math comments anchored to SM:718, preview marketLine '>=' → '≥' glyph (display-only; dup-hash/preflight untouched), below-target recorded as contracts-v2 deferral in if0-deferred-items.md; (2) webCreateCallSchema tightened ticker-only (LISTED_ASSET_MESSAGE on all branches — dead 0x-hex/numeric form path now rejected inline pre-preflight; AssetSelect made it UI-unreachable anyway), relayer-parity surface PROVEN byte-identical (resolve-asset/preflight-body files + suites untouched, 20/20 + 11/11). Gates: web 231/231, build 0. | 2026-06-11 | 9f15ac4 | [260611-if0-close-out-hog-findings-direction-honesty](./quick/260611-if0-close-out-hog-findings-direction-honesty/) |
| 260611-hog | Composer asset-selection UX upgrade (user-requested): (1) free-text asset input replaced with reusable AssetSelect dropdown — 24 allowlisted assets from shared PYTH_FEED_IDS in 6 optgroups (Majors/L2s/DeFi/Restaking/Memes/AI-RWA), value stays the bare ticker (preflight/dup-hash/preview unchanged), wired into PriceTargetFields + SpreadVsFields (A+B) + EventFields, label htmlFor added, catalog-drift test pins dropdown↔PYTH_FEED_IDS; (2) live current price under the select via public Pyth Hermes API — new pure lib hermes-price.ts (parse/format: 4 sig figs sub-$1, 2 decimals else) + usePythPrice hook (5s abort, 30s refresh, D-07 null degrade — fetch fails → row hidden, never fake); (3) price-target % quick-pick chips (+10/+20/+50/+100) render only when live price available, write through field.onChange→usdToTargetValue (1e8 scale preserved), manual entry intact. NOTE for future phases: no `direction` field exists in CreateCallInput (preview hardcodes ≥) so chips are above-only; EventFields hex/numeric event-asset entry lost its UI path (schema still accepts). Gates: shared build 0, web 228/228 (+22 new), web build 0; no Playwright rewrites needed. | 2026-06-11 | 236c552 | [260611-hog-composer-asset-dropdown-24-allowlisted-a](./quick/260611-hog-composer-asset-dropdown-24-allowlisted-a/) |
| 260611-h44 | Receipt page settled-state honesty fixes (the deferred half of the f7c review; 4 user-locked decisions via AskUserQuestion, all "recommended" picks) — settled branch of /call/[id] only: (1) zero-neutral stats: P&L "$0.00" + Rep Δ bare-zero render text-secondary with no "+" (three-way color; UI-45 count-up preserved); caller rep chip accent-win only when globalRep>0; (2) neutral PENDING RESULT no longer gets the giant stamp — dispute-pattern block (.h-2 slate "Pending result" + mono explainer), data-outcome-word Playwright hook preserved, real verdict words keep Stamp byte-for-byte; (3) unresolved caller: handleResolved gates avatar+name — neutral "?" avatar + mono "caller unresolved" instead of "#14" styled as identity (D-07); (4) SHARE AS FRAME demoted from outline button to mono text link "or share as a Farcaster frame ↗" under the cream primary (44px target, mobile full-width, .receipt-frame-link hover in globals.css). Gates: web 206/206, build exit 0, no spec assertions weakened. | 2026-06-11 | abe0a84 | [260611-h44-receipt-page-settled-state-honesty-fixes](./quick/260611-h44-receipt-page-settled-state-honesty-fixes/) |
...
| 260611-h36 | Relayer resilience overhaul (kills the Upstash/Graph quota outage class + makes auto-settlement real for the first time) — 4 commits (e03c6bd, cf4d844, 8937aa0, c8db145): (1) in-process L1 TTL cache (memory-cache.ts, bounded 500, stale-horizon 1h) + L1-first cache.ts adapter on all 8 Redis cache sites (TTLs/shapes preserved) + guards on the 5 hard-500 Redis paths (dup-check skip-cache, paymaster fail-closed deny + {count:null,degraded}, unlink best-effort, memory-nonce fallback durable across Redis recovery, admin 503); (2) subgraph circuit breaker in subgraph-client.ts (120s cooldown, last-known-good snapshots ≤1h, feed _source:'stale', infra-only classification — GraphQL/config errors bypass breaker, half-open single-probe gate, stale challenge-id warn); (3) NEW workers/settlement-poller.ts — Redis-FREE chain-polling settlement (getCall frontier discovery no-subgraph, Pyth-only v1, receipt-gated terminality, 30-attempt P0 + 10-min backoff, registry-sanity + frontier-stuck alerts, sanitizeError signer-address redaction); SETTLEMENT_SIGNER_PRIVATE_KEY optional via secret-manager — ABSENT = IDLE dry-run logging settlement_poller_would_settle; (4) BullMQ settlement worker GATED OFF (SETTLEMENT_BULLMQ_ENABLED default false — had NO producer; its 24/7 Redis polling = prime 500K-quota-burn suspect). Review 0C/6W/6I → all 6 W fixed (receipt-wait WR-01 the critical one for unattended settles). Tests 264→294 green, build green. ROOT-CAUSE DISCOVERY: auto-settlement was NEVER functional (enqueueSettlement zero callers + walletClient throw-stub) — every prior settle was an operator script. Operator notes in SUMMARY (env flags + signer secret + expected first-deploy logs; call #15 would-settle expected after 2026-06-12 10:08 UTC). | 2026-06-11 | c8db145 | [260611-h36-relayer-resilience-overhaul](./quick/260611-h36-relayer-resilience-overhaul/) |
| 260611-lks | Server-only SUBGRAPH_URL precedence for gateway key safety (D-27) — the subgraph moved to the production Graph gateway whose URL embeds the API key in its path (the key IS the URL), so both web subgraph readers now read server-only `SUBGRAPH_URL` first: leaderboard-client falls back to legacy keyless `NEXT_PUBLIC_SUBGRAPH_URL` then '', relayer-client (getSettledFields/getDuelSettledFields) falls back to the keyless `SUBGRAPH_URL_SEPOLIA` shared const (packages/shared untouched). Client bundles evaluating relayer-client's module-level read degrade safely to the keyless const (server vars never inlined; no client path calls the readers). 3 stale comment blocks rewritten; new subgraph-url-precedence.test.ts (5 tests: precedence behavioral via vi.stubEnv/resetModules + static env-first pin + gateway-host leak guard built from joined fragments — no host literal committed). Gates: web vitest 236/236, build 0. ⚠️ OPERATOR: add server-only `SUBGRAPH_URL` (full gateway URL with key) to the web Vercel project — NOT `NEXT_PUBLIC_*`; leave `NEXT_PUBLIC_SUBGRAPH_URL` as the legacy keyless Studio URL. Until then behavior unchanged. | 2026-06-11 | 3a59cd6 | [260611-lks-server-side-subgraph-url](./quick/260611-lks-server-side-subgraph-url/) |
| 260611-m72 | Settings Save Handle honesty (user's username change vanished without feedback — cast-verified displayHandle still "" on both wallets) — handleSetDisplayHandle was fire-and-forget sync writeContract: NO chain guard (the ONE site missed by the 17-site sweep), NO error surfacing (rejection / 0-ETH gas / revert silently reset the button), success = submitted-not-mined. Fixed to the proven pattern: ensureActiveChain → writeContractAsync(chainId pinned) → waitForTransactionReceipt with receipt.status gate; honest error taxonomy (rejection copy / no-gas faucet copy / viem shortMessage) via existing inline handleError; "Handle saved on-chain." renders only post-mine; isSavingHandle spans the full flow. Web 236/236, build 0. | 2026-06-11 | 6d131b7 | [260611-m72-settings-save-handle-honesty](./quick/260611-m72-settings-save-handle-honesty/) |
| 260611-npv | ProfileRegistry writes were going to the ZERO ADDRESS (user's "test" handle save: Rabby + receipt both reported success, displayHandle still "" on canonical 0xF66C0AFE AND old 0xE82308B3 — a tx to 0x0 has no code → no revert → receipt.status 'success', so even m72's receipt gate passed while writing into the void). Root cause: settings page + SocialLinkControls each had local `process.env.NEXT_PUBLIC_PROFILE_REGISTRY_ADDRESS ?? '0x000…000'` consts and Vercel lacks the env. Fix: both files now import canonical PROFILE_REGISTRY_ADDRESS from @/lib/chain (never zero); setDisplayHandle save adds post-receipt READ-BACK verification (readContract displayHandle(caller) must equal the submitted input before "Handle saved on-chain."); unlinkTwitter/unlinkFarcaster repointed. chain-pinning.test.ts +4 pins (non-zero active addr, both files off the env-fallback pattern, read-back wiring). Web 240/240, build 0. Operator: no Vercel env needed anymore for these surfaces. | 2026-06-11 | 75ab468 | [260611-npv-profile-registry-canonical-addr](./quick/260611-npv-profile-registry-canonical-addr/) |
| 260611-obx | Live-call share controls on /call/[id] (user-reported: no share button after making a call — share intents were settled-only; the prototype's live SHARE was cut UNWIRED in 09.2 D-09, and usePublishCall redirects straight here). Live-scope derivation mirrors the settled path via the same @call-it/shared pure builders: NEXT_PUBLIC_OG_BASE_URL → /call/{id} URL (unfurls as the Phase-7 LIVE OG card) → honest head 'LIVE CALL' (genuinely live) / 'ON RECORD' (awaiting-settlement + callerExited fall-through; win words unreachable, D-08/08-05 GAP 1) → buildShareText (raw displayHandle; isRealHandle self-filters 0x/#N fakes) → X web intent (cream "SHARE THIS CALL →") + demoted Farcaster compose mono link, `data-live-share-row` in the live header spread, rel=noopener, minHeight 44, mobile full-width; omitted entirely when OG base unset (no dead controls, D-08). Settled branch byte-identical. New live-call-share.test.ts (5 vitest source assertions: builders in live block, both heads, no win word in derivation, tabnabbing contract, stale cut-comment gone). Web 245/245, build 0. | 2026-06-11 | fbb7aec | [260611-obx-live-call-share](./quick/260611-obx-live-call-share/) |
| 260611-p9a | Profile identity + latency, 2 commits (user-reported: username still "@0x7304...5ced" after a SUCCESSFUL save — cast shows "test" on-chain — and profile clicks take ~6s; live-probed: relayer /api/profile returned displayHandle:"" source:truncated in 5.86s with the cache never filling). (1) b6d825e relayer: profile.ts getProfileRegistryAddress() was `NEXT_PUBLIC_PROFILE_REGISTRY_ADDRESS ?? 0x0` (env never set on Fly → every displayHandle/settledCalls read hit the ZERO address → caught → ''/0n → truncated fallback; same landmine class as web npv) → now canonical PROFILE_REGISTRY_ARBITRUM_SEPOLIA from @call-it/shared (notification-fanout precedent); ens-resolver.ts resolveEns now returns null IMMEDIATELY when ENS_MAINNET_RPC_URL unset (was `http(undefined)` → viem default public mainnet RPC hangs from Fly → 5s leg burn EVERY request, and CR-01 rightly never caches timed-out resolutions → 60s cache never filled → every click paid ~6s; configured path byte-identical, CR-01 untouched). +2 test files (5 tests) + pre-existing ens suite env save/restore (0 assertions changed). Relayer 324/1skip, tsc 0. (2) 5d8efc4 web: settings handle input normalized `trim().replace(/^@+/,'')` through validation/tx-args/read-back/input-sync (typing "@test" would have stored the @ on-chain → "@@test" display) + help copy; chain-pinning +4 pins. Web 249/249, build 0. ⚠️ relayer fix INERT until Fly redeploy; optional ENS_MAINNET_RPC_URL on Fly re-enables ENS names (absence now fast+honest). | 2026-06-11 | 5d8efc4 | [260611-p9a-profile-identity-latency](./quick/260611-p9a-profile-identity-latency/) |
| 260611-o5b | Alchemy CU burn fix (relayer event watchers were burning ~6M CU/day — 9-block getLogs chunking × 4 streams, 24/7 → 30M free tier gone in ~5 days): NEW shared workers/chain-scanner.ts — ONE block-cursor loop, ONE getLogs per window (address array FFM CallerExited + SM CallSettled + PR SocialUnlinked), 500-block windows (env ladder CHAIN_SCANNER_BLOCK_SPAN > NOTIFICATION_FANOUT_BLOCK_SPAN > 500; max-windows cap 50), corrected the stale "10 blocks per request" free-tier comment. notification-fanout / social-unlink-watcher / auto-post-worker private scan loops deleted → scanner registrations; public start*() APIs, getStats(), dedup, WR-05 never-scan-from-block-1 guard, error resilience all byte-compatible (auto-post test suite passed with ZERO edits via private-scanner fallback). index.ts wires one shared instance (onReady start / onClose stop). ~4 getLogs streams → 1 at ~55× wider windows ≈ ~200× fewer getLogs. NO UX change (same events, same 30s tick; web 5s polls untouched; settlement-poller/polled-events-fallback/duel workers untouched). Scanner 25 tests new; relayer 319/1 skip, build 0. ⚠️ Takes effect at next relayer Fly redeploy (same deploy h36 needs). | 2026-06-11 | b926703 | [260611-o5b-alchemy-rpc-burn](./quick/260611-o5b-alchemy-rpc-burn/) |
| 260611-sof | Subgraph globalRep staleness fix (09.2 UAT finding 1: leaderboard showed losers unpunished at 100) — root cause: SM._computeRepDelta emits RepCalculated with the PRE-applyRepDelta currentRep (SM:282 read / :311 emit) and the mapping persisted it. Fix: Profile.globalRep now mirrored EXCLUSIVELY from ProfileRegistry.RepDeltaApplied newRep (the single mutator's POST-apply value — REP-02 floor + WR-08 clamp baked in; covers settle/exit/duel/dispute-reversal for free, duel-rep "known gap" CLOSED); stale write DELETED from handleRepCalculated (RepDeltaApplied fires earlier in the same tx — keeping it would clobber). New rep-mirror.test.ts source-assertion suite (TDD RED→GREEN; single-source-of-truth writer census). Studio v0.9.2 DEPLOYED + synced (hasIndexingErrors=false) + HARD acceptance gate PASSED: loser 0x3e6c1e…=90, treasury 0xda8c…=77, spot=100 — byte-identical to cast on PR 0xF66C0AFE. SUBGRAPH_URL_SEPOLIA → v0.9.2; .env.local NEXT_PUBLIC bumped (unstaged). ⚠️ OPERATOR: Vercel NEXT_PUBLIC_SUBGRAPH_URL + server-side SUBGRAPH_URL (the DN gateway URL points at OLD stale mappings and leaderboard-client PREFERS it) + Fly RELAYER_SUBGRAPH_URL — exact commands in SUMMARY.md. | 2026-06-11 | fce74ca | [260611-sof-subgraph-globalrep-staleness](./quick/260611-sof-subgraph-globalrep-staleness/) |
...
| 260611-fast-x-coming-soon | (gsd-fast) "From your X" feed section COMING SOON state — product decision: follows.read needs a PAID X API tier, not buying now; the opted-in empty state ("No active calls from people you follow on X yet.") was misleading since the keyless relayer ALWAYS returns empty. FromYourNetworkSections.tsx: X_FOLLOW_GRAPH_LIVE=false module gate → X section body renders `pill warn` COMING SOON + honest mono copy, /api/feed/from-your-x fetch skipped while off; AUTH-16 opt-in gate + Farcaster section untouched. Flip the const to true when X_API_BEARER_TOKEN lands on the relayer. Web 249/249, build 0. Live on next Vercel push. | 2026-06-11 | 4b2ea01 | — |
| 260611-scj | Wallet pill popover (user request: "click on your balance → address popup + small profile details"): header balance pill extracted to WalletPill.tsx and upgraded from inert div to `<button aria-haspopup="dialog" aria-expanded>` toggling an anchored neobrutalist panel (absolute right-0 below pill, 2px border + 4px hard shadow, z-60). Contents D-07-gated: as-stored @handle headline + VERIFIED·X/FC pills (skipped when source==='truncated' → address becomes headline), always-on address row `0x1234…abcd` + COPY→COPIED(2s) clipboard button (fund-page pattern), 2dp balance row, stats line (N calls · N settled · N wins · REP n), VIEW PROFILE → /profile/{addr} + SETTINGS → /profile/{addr}/settings links closing on navigate. Escape + wrapper-contains outside-click close, listeners attached only while open. AUTH-44 refined (user decision 2026-06-11): pill FACE never shows an address; popover shows the viewer's OWN address only after deliberate click — panel JSX gated `open && profileAddr &&`, proven by index-order test (no address slice before the gate). No new fetching (reuses pill's existing useAccount/useUsdcBalance/useProfile). globals.css button.wallet-pill reset + hover; AppShell C10 search block byte-stable. New wallet-popover.test.ts (8 source assertions). Web 257/257, build 0. Live on next Vercel push. | 2026-06-11 | 6d18153 | [260611-scj-wallet-pill-popover](./quick/260611-scj-wallet-pill-popover/) |
| 260611-fast-settled-tape-outcome | (gsd-fast, 09.2 UAT finding 3) Settled tab tape cards now show the §15.7 outcome word inline — /api/feed already returned outcome (CallerWon/CallerLost, live-verified) but CallCard only rendered a muted SETTLED tag. CallCardData += outcome; settled branch renders Tag win "CALLED IT" / Tag loss "LOUD AND WRONG" / muted SETTLED when absent (D-07 never guess); FeedList passes it; FeedItem typed (wire passthrough via spread, boundary untouched). ui 82/82+build 0, web 257/257+build 0. Live on next push. | 2026-06-11 | 7e33294 | — |
| 260611-fast-mobile-header-slim | (gsd-fast) Mobile header fits the viewport (user report: "be right in public" tagline pushed the wallet balance out of the viewing width on mobile, and the pill face showed the handle): brand slash+tagline now `{!isMobile && …}` (desktop unchanged); WalletPill face is balance-only — handle removed from the button face, still the popover headline (face stays narrow; AUTH-44 face contract comment updated in both files). +1 face-balance-only source pin (no `{handle}` before the open gate, `@{handle}` after). Web 258/258, build 0. Live on next Vercel push. | 2026-06-11 | 41b3810 | — |
| 260611-t7h | Feed tabs restored + filter chips (user request w/ prototype screenshot — supersedes the 09.2 D-08 cut now that real wiring exists): home feed tab row is Live calls/Settled/Following/Duels with prototype `.count` chips (Live/Settled = loaded lengths; Duels = real /api/duels count only post-fetch-success; Following = NO count, the prototype's 12 was fake — D-07). DUELS tab: one-shot /api/duels fetch (8s abort → null) rendering compact rows → /duel/{id}, empty/failed → dashed "NO LIVE DUELS IN YOUR GRAPH." FOLLOWING tab: hosts FromYourNetworkSections (moved off Live — pure tape now) with new `fallback` prop → dashed "QUIET HERE." when both sections hide. CHIP ROW (Live+Settled only): All/Majors/DeFi/L2s/Memecoins/Arbitrum Eco/Restaking via new lib/asset-class.ts membership test over REAL assetSymbol enrichment (NOT the 3-value on-chain Category enum; ARB in both L2s+Arb Eco; unmapped→All only; NFTS/MACRO cut per D-08 — no such call types in v1); chip-empty → mono "NO {CHIP} CALLS ON THE TAPE." New feed-tabs-chips.test.ts (source+unit pins). Web 276/276, build 0 (orchestrator-verified — executor socket dropped twice mid-run, committed work intact). Live on next Vercel push. | 2026-06-11 | d0e3161 | [260611-t7h-feed-tabs-chips](./quick/260611-t7h-feed-tabs-chips/) |
| 260611-tbc | Settled tape card redesign (operator screenshot + prototype feed.jsx SettledCard recipe; supersedes the 7e33294 win/loss pill): new @call-it/ui SettledCallCard — bracketed brutal card, huge top-right outcome word w/ 3px 3px 0 #000 hard offset shadow + per-word colors (CALLED IT=accent-win, LOUD AND WRONG=accent-loss, CONTRARIAN HIT=accent-duel, COLD CALL=text-tertiary; absent outcome → muted SETTLED tag, D-07), `settled <date> · <time> UTC` overline, FINAL/REP Δ/STAKE label-overline stat blocks, outlined SHARE anchor (X intent via shared builders, raw-handle isRealHandle guard, stopPropagation preserves D-06 card-tap nav, env-gated on NEXT_PUBLIC_OG_BASE_URL). Relayer /api/feed additively enriches settled items (ONE batched settlements+repEvents query through the breaker, fail-safe empty map, never throws): settledAt, caller-filtered repDelta (stricter than OG unfiltered first:1), finalPct ONLY for marketType 0 (SM.sol:713-723 priceDelta=final−target both 1e8 → signed % past/short of target; types 1/2 excluded — governance priceDelta=0 would fake 0%). Web degrades gracefully vs CURRENT deployed relayer (no wire fields → word+statement+STAKE+SHARE only). Gates: ui 104/104, web 283/283, relayer 334/0 fail (ens-resolver did not reproduce), 4 builds green. ⚠️ stats row INERT until operator relayer Fly redeploy + web push. | 2026-06-11 | 64fd6b7 | [260611-tbc-settled-tape-card-redesign](./quick/260611-tbc-settled-tape-card-redesign/) |
| 260611-u1l | Live call card redesigned to the prototype (user screenshot: jaxon.eth ARB/OP card — "intelligently mix what we have"): new apps/web LiveCallCard renders live tape items in the prototype layout with every element wired to REAL data — square grad avatar + handle (displayHandle ?? handle ?? NEW on-chain ProfileRegistry.displayHandle multicall ?? truncated caller, rendered as stored), CLOSES IN ticking countdown + AWAITING SETTLEMENT C2 parity, Archivo statement, asset-class + symbol pills (new assetClassesFor over real assetSymbol), CONVICTION row + real follow/fade odds bar + pool dollars from NEW one-shot FollowFadeMarket followReserve/fadeReserve multicall (call-page formulas mirrored; whole block reserves-gated D-07), FOLLOW/FADE/CHALLENGE → /call/{id} + quote → /new?quote={id} nav Links (D-06, no counts — none exist), footer = truncated caller · posted-ago (no block/challenges — no source). USER CONSTRAINTS enforced + test-pinned: NO stake amount, NO VERIFIED CRITERIA badge. Omitted as sourceless (D-07): rep badge, verified pills, position counts. Hooks chainId-pinned, staleTime-only, ZERO refetchInterval (Alchemy CU). Settled path untouched (260611-tbc owns it). New live-call-card.test.ts (20 pins). Web 303/303, build 0. Live on next Vercel push. | 2026-06-11 | 59e08aa | [260611-u1l-live-call-card](./quick/260611-u1l-live-call-card/) |
| 260611-uf9 | Trivially-true call guard (user: “ETH ≥ $37 when ETH is ~$2,500 shouldn’t be allowed” — guaranteed CALLED IT = rep farming): priceTarget calls must target STRICTLY ABOVE the live Pyth price (V1 settles ≥ only, no direction field; spreadVs/event untouched, no margin band — barely-above is a legit coin-flip). TWO LAYERS: (1) web composer — new pure target-guard.ts; usePythPrice lifted to page.tsx (one fetch serves field display + modal gate); PriceTargetFields takes price as prop and renders a derived reactive inline error; onPublish blocks the confirm modal while violated; null/error price → client check skipped (D-07, server enforces). (2) relayer preflight — new hermes-spot.ts (exact 1e8 expo normalization multiply/floor paths, never-throw getSpotPrice1e8), calls-preflight.ts gates priceTarget under 4s withTimeout, 422 D-31 field errors target_not_above_current / price_unverifiable; FAIL-CLOSED when Hermes unreachable. On-chain createCall bypass accepted residual (contracts-v2). Gates: web 312 + build 0, relayer 350/1 pre-existing skip + tsc 0, ui untouched. ⚠️ relayer guard INERT until Fly deploy; web guard live on next push. | 2026-06-11 | e3ac074 | [260611-uf9-target-above-current-guard](./quick/260611-uf9-target-above-current-guard/) |
| 260611-fast-awaiting-actions | (gsd-fast) Awaiting-settlement feed cards no longer show FOLLOW/FADE/CHALLENGE (user: “one can't follow, fade or challenge them so the buttons shouldn't be there” — contracts reject post-expiry positions/challenge creation, dead controls per D-08): LiveCallCard action Links gated on !isAwaitingSettlement (already-derived ticking flag); quote button stays — quoting an expired call is still a valid new-call affordance (marginLeft:auto keeps it right-aligned when alone). Call page already correct (B6). +2 source pins (gate order + quote-outside-gate). Web 337/337, build 0. Live on next Vercel push. | 2026-06-11 | e7f20d4 | — |
| 260612-fast-force-settle-script | (gsd-fast) force-settle-stuck.ts ops script (user approved option 1): owner forceSettle for the 5 stuck seeded Event calls #3–#7 (empty criteria, non-Pyth rail not live — poller skips by design). Cooldown unlocks **2026-06-13 11:16 UTC** (expiry+7d); forced outcome is ALWAYS CallerLost (SM.sol:400). Rails: DEPLOYER_PRIVATE_KEY signer hard-refused unless == SM.owner(); already-settled + cooldown-active skips; receipt success asserted. Run from apps/relayer: `npx tsx src/scripts/force-settle-stuck.ts`. tsc 0. | 2026-06-12 | fb2e48a | — |
| 260612-fast-leaderboard-sub | (gsd-fast) Leaderboard subtitle removed (user request): the “Reputation is a function of accuracy × volume. Easy to climb. Easy to fall.” .sub div cut from LeaderboardClient.tsx — Top of Book h1 stands alone. Web 380/380, build 0. Live on next Vercel push. | 2026-06-12 | 8058368 | — |
| 260611-ust | Duels surface upgraded (user: cards "too basic", wanted at-a-glance data + live/settled sections like the tape): /duels reworked — LIVE DUELS / SETTLED DUELS tabs (tape .tabs recipe, real post-fetch counts, independent per-tab fetch/error; settled = ?status=Settled), NEW DUEL KING banner from the wire's duelKing field (was silently dropped; handle via ProfileRegistry multicall; D-07 hidden when null), bare 4-col brutal-table replaced by rich DuelCard list. NEW shared DuelCard (also swapped into the feed's Duels tab, replacing DuelRowLink): status chip (LIVE DUEL/AWAITING ACCEPT/SETTLED) + TRENDING + #id·proposed-ago, market line via per-duel /api/duels/:id/live-state → callId → existing getMarketLine (THE "what's this about" line), challenger-vs-caller VS row (purple/cream, avatar grads, real handles via useFeedHandles fallback truncated, per-side stakes), POT + status clock (Accepted → ticking CALL CLOSES IN from parent-call expiry / AWAITING SETTLEMENT; Proposed → 24h ACCEPT WINDOW countdown citing ChallengeEscrow.sol:59), RIDING CALLER/CHALLENGER consensus split bar from live-state reserves (===0n→50 guard), WINNER row (Settled + non-zero winner only — winner-side accent + TOOK pot). All enrichment one-shot (cap 20, zero polling), every zone D-07 degrade-to-hidden. NEW lib/duels-client.ts single wire source (both pages' duplicated fetch/helpers lifted); D-15 pin RELOCATIONS in feed-tabs-chips + presentation-sweep (same contracts, new homes, cited). duels-surface.test.ts (23 pins). Web 335/335, build 0. Live on next Vercel push. | 2026-06-11 | fdac4fe | [260611-ust-duels-surface](./quick/260611-ust-duels-surface/) |
| 260611-fast-ticker-removal | (gsd-fast) SystemTicker REMOVED per user ("we're removing that system ticker"): component file deleted, AppShell import/flag/render dropped, globals.css ticker recipes + tickerScroll keyframes + --bg-ticker token deleted (liveDot kept — used app-wide); --shell-offset pinned '0px' (the .app-header/.layout/.sidebar calc() consumers unchanged, D-11 note updated in both files). No test pins existed (verified). Web 335/335, build 0. | 2026-06-11 | 54a7dc6 | — |
| 260611-vob | Duel page wired to real data (user prototype screenshot — "this is how the duel page should look"; root cause: the 09.2-11 prototype markup was already there but the duel-live-state wire never carried handles/rep/positions/symbols, so the page rendered "—/—" + truncated addresses + zeros): hero now marketType-driven from a NEW chainId-pinned CallRegistry.getCall read + NEW lib/feed-symbols.ts (PYTH_FEED_IDS inversion mirroring relayer call-enrichment canon) — RelativePerformance → giant "A/B" pair, PriceTarget → giant "A", else marketLine headline (never a fabricated pair); subtitle + caller POSITION box = authoritative marketLine reused from the EXISTING riders live-state fetch (zero new requests); challenger POSITION = literal "TAKES THE OTHER SIDE." (contract semantic — no challenger statement rail exists, verified); identities via useProfile ×2 — handles AS STORED w/ truncated fallback, VERIFIED·X/FC pills, RepBadge-recipe rep chips, REP stat (isFinite-gated) + ACCURACY (settledCalls>0-gated, never 0%-from-no-data); SETTLES IN gains honest oracle sub (PYTH ORACLE / ATTESTED EVENT by marketType); LIVE SPREAD stays D-07-hidden (no source). USER CONSTRAINT: win-streak blocks + fields fully deleted; IN CATEGORY column removed (fake-data surface stays dead). Handlers/consensus/riders/5s liveness byte-preserved; presentation-sweep C6 pin honestly migrated (D-15). duel-page-data.test.ts (20 pins). Web 357/357, build 0. Deploys on this push. | 2026-06-11 | 9763b9a | [260611-vob-duel-page-data](./quick/260611-vob-duel-page-data/) |
| 260612-8wk | HOW IT WORKS on the tape (user request, Polymarket-style): ghost "HOW IT WORKS" button in the page-header LEFT of + NEW CALL (renders signed-in AND signed-out — static education) opening a D-13 cream modal-panel explainer — overline · HOW IT WORKS, "Three steps. One receipt." heading, the 3 landing-page steps VERBATIM (GO ON RECORD / FOLLOW OR FADE / GET YOUR RECEIPT — local copy with signin/page.tsx cited as canon + a lockstep test pinning both files so copy drift forces a conscious sync), real-constants footnote ($5 MIN · $100 MAX PER CALL · 1.7% SETTLEMENT FEE), black MAKE YOUR FIRST CALL ▸ CTA wired to the same auth branch as the header CTA. Escape/backdrop/✕ close, role=dialog + aria-modal, 44px targets, zero data fetches (static-honesty pin). page.tsx surgical (import/state/button/mount — pinned regions byte-identical). how-it-works-modal.test.ts (9 pins). Web 366/366, build 0. Live on this push. | 2026-06-12 | 6a3d792 | [260612-8wk-how-it-works-modal](./quick/260612-8wk-how-it-works-modal/) |
| 260612-a6v | HOMEPAGE REPLACED (user request — "replace the homepage that people see when they visit for the first time" with `call it homepage/CALL IT Hero.dc.html`): /signin rewritten to the acid hero — #D4F500 frame, #0A0A0A rounded panel, glass-column + bloom atmosphere (ci-pulse/ci-bloom), Archivo Black headline BE RIGHT / IN PUBLIC. clamp(64px,8.6vw,124px), 3 staggered glass demo cards verbatim (veda/jaxon.eth/degen_oracle — decorative marketing art, header-documented D-07-exempt). USER EDIT: Market/Leaderboard/Dashboard nav pills CUT → single "How it works" pill opening the existing HowItWorksModal (onPrimaryCta → signup). Sign In → + MAKE YOUR FIRST CALL → open an ALWAYS-MOUNTED display-toggled dark-glass signin modal hosting the verbatim Privy rail (PrivyErrorBoundary + CustodyTooltip + dynamic SignInButtons + AUTH-37 disclaimer) — never conditional-rendered, so the SignInButtons cookie self-heal effect (SignInButtons.tsx:114-153) still runs on page load (regex-guarded in landing-hero.test.ts). See Live Calls → NEW public /calls route (`export { default } from '../page'`) — public via the EXISTING middleware '/call' prefix startsWith, zero middleware change (dependency pinned); / stays gated so first visits land on the new homepage. Logo via static import (raw /brand/ URL would 307-bounce through middleware — documented). Fonts page-local next/font (Archivo Black + Archivo 500-800). Test relocations (D-15 cited): signin.spec clamp+badge pins migrated, Tier 2 modal-open steps + signin-modal-buttons D-33 scoping; how-it-works lockstep re-anchored modal-as-single-canon + landing-mount linkage; landing-hero.test.ts NEW (9 pins). Visual-smoke signin.png baseline invalidated (operator regen, box-gated). Web 376/376, build 0. Live on this push. | 2026-06-12 | 880c5cb | [260612-a6v-homepage-hero-replace](./quick/260612-a6v-homepage-hero-replace/) |
| 260612-fast-landing-mobile | (gsd-fast) Landing mobile rework (user: "the mobile view can be better, fix it intelligently") — preview-measured at 320/375/768/1280 before+after (eval geometry; screenshots time out in this preview: the renderer freezes animation clocks on the page's infinite ci-pulse/ci-bloom). FOUND at 375: brand wordmark + Sign In wrapped to 2 lines (55/60px tall), 64px h1 floor = 29% of viewport + overflow ≤350px, badge overflow ≤330px, ragged unequal wrap-centered CTAs, no dvh, sub-44px taps. FIX: every media-targeted property moved inline→ci-* classes (inline beats stylesheet — the prior breakpoint was half-dead), then 3 tiers: ≤860 cards stack center-first as ±1.4° deck; ≤640 two-row nav (brand+Sign In top, How-it-works pill centered under w/ glass chrome collapsed), h1 clamp(40px,14.8vw,64px), badge wrap+tighten, full-width stacked CTAs, 44px targets, dvh page/panel, modal maxHeight 86dvh+overflow+24px pad; ≤360 type squeeze. Desktop byte-identical values verified (h1 110px@1280, glass pill restored, ±2.5°/26px/-18px card transforms exact via transition-kill diagnostic). Verified live: no overflow 320-1280, modal opens/fits/closes, SignInButtons mounts while modal closed (self-heal invariant). All spec pins survive (clamp string kept in .ci-h1). Web 376/376, build 0. | 2026-06-12 | (see hash) | — |
| 260612-fast-public-assets | (gsd-fast) Middleware bounced root-served public/ assets (latent bug found during a6v, VERIFIED LIVE: curl /icon.png + /splash.png on Vercel → 307 /signin): public-folder files serve at the ROOT URL, so the matcher's `public/` exclusion never matches them and they weren't PUBLIC_PREFIXES entries — the Farcaster crawler's unauthenticated fetches of farcaster.json's iconUrl/splashImageUrl (route.ts:55-56) + the cast-embed splash (farcaster-embed.ts:89) got HTML redirects, likely breaking Mini App icon/splash. FIX: 3 explicit entries (WR-03 discipline, no blanket) — '/icon.png', '/splash.png', '/brand/' (trailing slash so a future /branding PAGE is not auto-published; pinned by test). middleware-public.test.ts +5 cases (manifest images public, brand dir public, /branding NOT, source pins). Preview-verified on dev: all 3 → 200 image/png, /new control still bounces. Web 380/380, build 0. Live on this push. | 2026-06-12 | (see hash) | — |
| 260612-fast-landing-square | (gsd-fast) Landing radii zeroed (user: homepage rounded corners are off-brand — app is radius-0 brutal): ALL border-radius removed from /signin — 28px/22px panel, 999px nav pill + How-it-works + Sign In + badge + both CTAs, 18/20px demo cards, 8/9px avatars, 6px conviction chips, 99px split-bar segments, 20px signup modal, 50% pulse dot (now square, mirroring the app's .live-dot globals.css:157). Header comment records user delta 4 (design file's radii deliberately not ported). Preview-verified: every element computes 0px, full panel-subtree scan finds zero rounded elements; no test pins on radii. Web 380/380, build 0. Live on this push. | 2026-06-12 | (see hash) | — |
