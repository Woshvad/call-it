...
status: executing
stopped_at: "Phase 01.5 — 4/5 plans complete (01.5-02/03/04 + 01.5-05 closed as documented degraded-to-empty deferral); only 01.5-01 setRelayer live-tx gate open"
last_updated: "2026-06-07T09:16:25.000Z"
...
Plan: 5 of 5 (4 complete — incl. 01.5-05 documented-deferral closure; 1 operator-deferred: 01.5-01 setRelayer)

> **Phase 01.5 status (2026-06-06, updated):** 4/5 plans COMPLETE — **01.5-02** (relayer social-link service), **01.5-03** (VerifiedBadge + AUTH-10 invariant), **01.5-04** (link/unlink UI + AuthKitProvider + opt-in), and now **01.5-05** (FEED sections — closed **complete-with-documented-deferral**; SUMMARY written; commits `7960dce`/`d02e694`/`e1076a3`) all have SUMMARYs. 01.5-05 Task 1 (X API + Neynar keys) was resolved via the plan's own degraded-to-empty disposition — v1 ships with NO keys; live data activates with no code change once `X_API_BEARER_TOKEN` + `NEYNAR_API_KEY` are set (AUTH-14 deferred; AUTH-15/16/17/18 satisfied + tested). **ONE gate remains open — 01.5-01** (env surface DONE, commit `3fbf88d`): the **setRelayer** live owner-key tx on canonical PR `0xF66C0AFE…` from treasury `0xDa8c5726` (on-chain `relayer()` confirmed still `0x0`). No-key prep helper added — `apps/relayer/src/scripts/prepare-set-relayer.ts` (commit `9b41f0f`) derives the oauth-proof KMS address + emits the exact `cast` commands; the operator runs the tx. Phase verification/completion still SKIPPED until 01.5-01 clears. **Local DB:** ✅ DONE (2026-06-07) — provisioned a dedicated `callit-postgres` container on `127.0.0.1:5434` (role+db `call_it_relayer_sepolia`, password preserved from `.env.local`); `.env.local` POSTGRES_URL repointed `5433`→`5434`; all 6 drizzle migrations applied (`follow_graph` + `social_link_index` verified, `drizzle.__drizzle_migrations` count=6). The old `5433` target was the shared `vexa-postgres` container (no Call It role/db). Production still applies `0005` to Fly Postgres at deploy. Full relayer suite 189✓/1 skipped; relayer+web+ui+shared builds 0.
