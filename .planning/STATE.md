...
Last activity: 2026-06-11 -- Completed quick task 260611-if0: hog findings closed (≥-only direction honesty in composer copy + ticker-only web schema; below-target = contracts-v2 deferral). Earlier today: hog composer asset dropdown + live Hermes price + % chips; h44 receipt honesty fixes (f7c design review closed); fo1 follow/fade approve fix; h36 relayer resilience overhaul. MILESTONE: full web publish path WORKS live; relayer redeployed (co5 enum map + RPC fallbacks). Still pending: Upstash quota (operator; settlement worker DOWN — h36 poller awaits deploy), Alchemy key monthly capacity exhausted (public-RPC failover live in web+relayer).
