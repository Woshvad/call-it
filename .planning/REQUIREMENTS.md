...
- [ ] **AUTH-27**: Paymaster sponsors exactly the first 5 transactions per user; 6th transaction onward requires user-provided ETH (§10.7, App.A.1)
- [ ] **AUTH-28**: Paymaster per-account 5-tx cap is enforced server-side at the paymaster gating layer, not just contract-side (§10.7)
- [ ] **AUTH-29**: When per-account cap is exhausted, user is routed to a "fund your wallet to continue" flow (§10.7)
