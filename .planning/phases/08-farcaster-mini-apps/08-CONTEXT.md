...
- **D-06:** **Settled-cast button triplet = `Follow-person` · `Challenge` · `Quote`** — all three remain on the settled Frame to keep it interactive, but they are **NOT all one-tap on-chain** (research correction, see D-06a). This replaces `Fade`, which is meaningless once a call has settled.
- **D-06a:** **RESOLVED 2026-06-08 (post-research):** Of the settled triplet, only **`Challenge`** is a real one-tap Frame transaction (`ChallengeEscrow.proposeChallenge`). **`Follow`** (social-follow) has **no on-chain function** — it is an off-chain relayer `follow_graph` op (Privy/SIWF-gated `/api/social/*`), which a Frame `tx` button cannot invoke; and **`Quote`** (`CallRegistry.createCall` + `parentCallId`) needs 12 market params + ~$15 fee/stake, so it is not fixed one-tap calldata. **Decision: `Follow` and `Quote` route to the existing "Open in Call It" deep-link** (Follow → caller profile / in-app social-follow; Quote → pre-filled new-call form), while `Challenge` stays a one-tap Frame tx. This amends D-06's "all three are real onchain actions" to "all three interactive: one on-chain (`Challenge`) + two deep-link (`Follow`, `Quote`)". The full in-frame on-chain-less social-follow (modern Mini App SDK SPA calling the relayer with the user's session) is a **Phase-10** enhancement, consistent with D-01/D-05's testnet-now / mainnet-Phase-10 split. [[D-07]]
