...
- Twitter Card Validator results (5/5): 🟢 **CARD META CONFIRMED LIVE 2026-06-08; only browser visual preview remains.** Curl ground-truth from `call-it-web-sepolia.vercel.app` AFTER the green GitHub auto-deploy (`a5357bc`): `/call/[id]` AND `/duel/1` both emit `summary_large_image` + absolute `og:image`; all OG images return 200 image/png (`/og/1` 49KB, `/og/duel/1` 39KB, `/og/fallback` 41KB). **DuelSettled meta was missing — fixed (quick `260608-ep8`, `57a402c`) and now live.** `cards-dev.twitter.com/validator` is dead (X shut it down 2022) — use X post-composer / opengraph.xyz for the browser visual preview. Remaining: browser visual preview of all 5 (Settled/Exited/DuelSettled *rendered images* also need seeded data — fold into the residual-2 seed run).
