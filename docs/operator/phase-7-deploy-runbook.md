...
- 200px outcome-word baselines (SC1) + authoritative `verify-event-coverage.ts` (OPS-04 live): ✅ **DONE 2026-06-08 (4/5; CONTRARIAN HIT deferred).** Seed: `seed-loss-call.ts` (quick `260608-lwe`/`260608-kqh`-fix `77e32ad`) created call **#14** (PriceTarget, target $1M 8-dp → guaranteed CallerLost), settled via `settle-pyth-calls.ts 14` → `CallerLost`. Baseline sources (committed): CALLED IT=`/og/8`, COLD CALL=`/og/11`, LOUD AND WRONG=`/og/14`, FADED CORRECTLY=`/og/14?as=fader`. 4 baseline PNGs generated against the deployed endpoint + committed (`OG_200PX_BASELINES=1 --update-snapshots`); all 4 words legible at 200px (eyeballed). OPS-04 coverage PASSED (call 14 indexed 265ms).
  - **CONTRARIAN HIT — DEFERRED (Phase 7 SC1 known gap):** the OG route `apps/web/app/og/[callId]/route.ts` hardcodes `fadeRealShare: 0`, and `outcome-word.ts` requires `fadeRealShare >= 0.5` for CONTRARIAN HIT, so the card cannot render that word regardless of seeded data. FOLLOW-UP: wire `fadeRealShare` from subgraph fade/follow positions in `getSettledFields`/the OG route, then seed a callerWon call with majority real fade and add its baseline. Not a seeding problem; tracked here.
- Incognito hydration spot-check (residual 6c): ⏳ **OPERATOR-PENDING** (browser) — open `/call/14`, `/profile`, `/leaderboard` in an incognito window; confirm they render without hydration errors. Closes the parked Phase-4 UAT-1/2/3. CORS + 200s already curl-confirmed.
